...
export default function PendaftaranSidangPage() {
  ...
  const fetchNilai = async (sidangId: number) => {
    try {
      ...
      const response = await request<{ data: Nilai[] }>(
      ...
      setError(
        err instanceof Error
          ? err.message
          : 'Failed to fetch assessment scores',
      );
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      setError('');
      const taResponse = await request<{
        status: string;
    ...
    return (
      <div>
        ...
        >
          ...
          <p>
            <strong>Admin Verification:</strong> {reg.status_verifikasi}
          </p>
          <p>
            <strong>Supervisor 1 Approval:</strong> {reg.status_pembimbing_1}
          </p>
          <p>
            <strong>Supervisor 2 Approval:</strong> {reg.status_pembimbing_2}
          </p>
          {reg.catatan_admin && (
            <p>
              <strong>Admin Notes:</strong> {reg.catatan_admin}
            </p>
          )}
          ...
          {reg.status_verifikasi === 'terverifikasi' && (
            <button
              onClick={() => fetchNilai(reg.id)}
              disabled={viewingNilaiFor === reg.id}
            >
              {viewingNilaiFor === reg.id
                ? 'Loading Scores...'
            ...
            <div style={{ marginTop: '1rem' }}>
              ...
              {nilaiSidang.length > 0 ? (
                <ul>
                  {nilaiSidang.map((n) => (
                    <li key={n.id}>
                      <strong>{n.aspek}</strong> by {n.dosen.user.name}:{' '}
                      {n.skor} - <em>{n.komentar}</em>
                    </li>
                  ))}
                </ul>
              ) : (
