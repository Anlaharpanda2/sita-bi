...
export default function TugasAkhirPage() {
  ...
  const handleApply = async (topicId: number) => {
    ...
    try {
      ...
      alert(
        'Successfully applied for the topic! Please wait for lecturer approval.',
      );
  ...
  if (tugasAkhir) {
    ...
    return (
      <div>
        ...
        <p>
          <strong>Title:</strong> {tugasAkhir.judul}
        </p>
        <p>
          <strong>Status:</strong> {tugasAkhir.status}
        </p>
        <h4>Supervisors:</h4>
        <ul>
    ...
    return (
      <div>
        ...
        <p>
          You do not have an active final project. You can apply for an existing
          topic or propose your own.
        </p>
          ...
          <tbody>
            {availableTopics.map((topic) => (
              <tr key={topic.id}>
                ...
                <td>
                  <button onClick={() => handleApply(topic.id)}>Apply</button>
                </td>
              </tr>
            ))}
          </tbody>
