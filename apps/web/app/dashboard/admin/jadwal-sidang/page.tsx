    ...
    try {
      ...
      setError(
        error?.response?.data?.message ||
          'Gagal membuat jadwal. Periksa kembali isian Anda.',
      );
    } finally {
      setSubmitting(false);
    }
      ...
      <div className="bg-white p-8 rounded-lg shadow-xl w-full max-w-2xl">
        ...
        <p className="mb-2">
          ...
          <span className="font-semibold">
            {registration.tugas_akhir.mahasiswa.user.name}
          </span>
        </p>
        <p className="mb-6">
          Judul:{' '}
          <span className="font-semibold">
            {registration.tugas_akhir.judul}
          </span>
        </p>
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            ...
            <div>
              ...
              <select
                value={ruanganId}
                onChange={(e) => setRuanganId(Number(e.target.value))}
                required
                className="mt-1 block w-full rounded-md border-gray-300 shadow-sm"
              >
...
export default function JadwalSidangPage() {
  ...
  const [selectedRegistration, setSelectedRegistration] =
    useState<ApprovedRegistration | null>(null);

  const fetchData = async () => {
    setLoading(true);
    try {
      const [regResponse, ruanganResponse, dosenResponse] = await Promise.all([
        api<{ data: { data: { data: ApprovedRegistration[] } } }>(
          '/jadwal-sidang/approved-registrations',
        ),
      ...
      setError(
        'Gagal memuat data. Pastikan Anda memiliki hak akses yang sesuai.',
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <p className="mb-6">
        Daftar pengajuan sidang yang telah disetujui dan siap untuk dijadwalkan.
      </p>
