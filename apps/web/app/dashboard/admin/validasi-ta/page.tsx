      ...
      <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-lg">
        ...
        {result ? (
          <ul className="list-disc list-inside">
            {result.map((r, i) => (
              <li key={i}>
                Dokumen mirip dengan <strong>{r.similarTo}</strong> (skor:{' '}
                {r.score.toFixed(2)})
              </li>
            ))}
          </ul>
...
export default function ValidasiTaPage() {
  ...
  const [kemiripanResult, setKemiripanResult] = useState<
    KemiripanResult[] | null
  >(null);

  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await api<{ data: { data: TugasAkhir[] } }>(
  ...
  const handleApprove = async (id: number) => {
    if (!window.confirm('Apakah Anda yakin ingin menyetujui tugas akhir ini?'))
      return;
    try {
  ...
  const handleReject = async (alasan: string) => {
    ...
    try {
      await api(`/tugas-akhir/${selectedTA.id}/reject`, {
        method: 'PATCH',
        body: { alasan_penolakan: alasan },
      });
  ...
  const handleCekKemiripan = async (id: number) => {
    try {
      const response = await api<{ data: { data: KemiripanResult[] } }>(
        `/tugas-akhir/${id}/cek-kemiripan`,
        { method: 'POST' },
      );
      setKemiripanResult(response.data.data);
      setKemiripanModalOpen(true);
    } catch {
      alert('Gagal mengecek kemiripan.');
    }
  };
  ...
  return (
    <div className="container mx-auto p-4">
              ...
              tas.map((ta) => (
                <tr key={ta.id} className="bg-white border-b">
                  <td className="py-4 px-6 font-medium">
                    {ta.mahasiswa.user.name} ({ta.mahasiswa.nim})
                  </td>
                  <td className="py-4 px-6">{ta.judul}</td>
                  <td className="py-4 px-6">
                    {new Date(ta.tanggal_pengajuan).toLocaleDateString('id-ID')}
                  </td>
                  <td className="py-4 px-6 flex gap-2">
                    <button
                      onClick={() => handleApprove(ta.id)}
                    ...
                    <button
                      onClick={() => handleCekKemiripan(ta.id)}
      ...
      />
      <KemiripanModal
        isOpen={isKemiripanModalOpen}
        onClose={() => setKemiripanModalOpen(false)}
        result={kemiripanResult}
      />
    </div>
  );
}
